...
import proyectoitq.RegistroUsuarioGUI;
import proyectoitq.RegistroProductoGUI;
...
//lo mismo pero con el texto de introducción
    ...
    private JPanel crearPanelCentral() {
        ...
        btnRegistrarProducto.addActionListener(e -> abrirRegistroProducto()); // Acción al hacer clic en "Registrar Producto"
...
//Si quitamos el createRigidArea los componentes se agruparían más estrechamente entre sí, lo que podría hacer que la interfaz se vea más "apretada" o desordenada
    ...
    private void abrirRegistroUsuario() {  //Colaboración con la clase Registro Usuario GUI
        new RegistroUsuarioGUI(this);
    }

    //lo mismo pero de producto
    private void abrirRegistroProducto() { //Colaboración con la clase Registro Producto GUI
        new RegistroProductoGUI(this);
    }
    ...
    private void mostrarInformacionUsuarios() {
        ...
        try (Connection connection = DriverManager.getConnection(jdbcURL)) {
            ...
            while (resultSet.next()) {
                ...
                modeloTabla.addRow(new Object[]{cedula, nombre, apellido, telefono, email});
                hayDatos = true;
            }
        ...
        botonActualizar.addActionListener(e -> {
            ...
            if (filaSeleccionada != -1) {
                ...
                String cedula = modeloTabla.getValueAt(filaSeleccionada, 0).toString();

                // Validar cada campo con un bucle para permitir reintentos
                String nuevoCedula = null;
                do {
                    nuevoCedula = JOptionPane.showInputDialog("Nueva Cédula:", modeloTabla.getValueAt(filaSeleccionada, 0).toString());
                    if (nuevoCedula == null) {
                        return; // Si el usuario cancela, salir del método
                    }
                    if (nuevoCedula.isBlank() || !nuevoCedula.matches("[0-9]+")) {
                        JOptionPane.showMessageDialog(null, "El campo 'Cédula' debe ser solo números.");
                    }
                } while (nuevoCedula != null && (nuevoCedula.isBlank() || !nuevoCedula.matches("[0-9]+")));

                String nuevoNombre = null;
                do {
                    nuevoNombre = JOptionPane.showInputDialog("Nuevo Nombre:", modeloTabla.getValueAt(filaSeleccionada, 1).toString());
                    if (nuevoNombre == null) {
                        return; // Si el usuario cancela, salir del método
                    }
                    if (nuevoNombre.isBlank() || !nuevoNombre.matches("[A-Za-zÁ-ÿ ]+")) {
                        JOptionPane.showMessageDialog(null, "El campo 'Nombre' debe ser texto y no puede estar en blanco.");
                    }
                } while (nuevoNombre != null && (nuevoNombre.isBlank() || !nuevoNombre.matches("[A-Za-zÁ-ÿ ]+")));

                String nuevoApellido = null;
                do {
                    nuevoApellido = JOptionPane.showInputDialog("Nuevo Apellido:", modeloTabla.getValueAt(filaSeleccionada, 2).toString());
                    if (nuevoApellido == null) {
                        return; // Si el usuario cancela, salir del método
                    }
                    if (nuevoApellido.isBlank() || !nuevoApellido.matches("[A-Za-zÁ-ÿ ]+")) {
                        JOptionPane.showMessageDialog(null, "El campo 'Apellido' debe ser texto y no puede estar en blanco.");
                    }
                } while (nuevoApellido != null && (nuevoApellido.isBlank() || !nuevoApellido.matches("[A-Za-zÁ-ÿ ]+")));

                String nuevoTelefono = null;
                do {
                    nuevoTelefono = JOptionPane.showInputDialog("Nuevo Teléfono:", modeloTabla.getValueAt(filaSeleccionada, 3).toString());
                    if (nuevoTelefono == null) {
                        return; // Si el usuario cancela, salir del método
                    }
                    if (nuevoTelefono.isBlank() || !nuevoTelefono.matches("[0-9\\s]+")) {
                        JOptionPane.showMessageDialog(null, "El campo 'Teléfono' debe ser solo números.");
                    }
                } while (nuevoTelefono != null && (nuevoTelefono.isBlank() || !nuevoTelefono.matches("[0-9\\s]+")));

                String nuevoEmail = null;
                do {
                    nuevoEmail = JOptionPane.showInputDialog("Nuevo Email:", modeloTabla.getValueAt(filaSeleccionada, 4).toString());
                    if (nuevoEmail == null) {
                        return; // Si el usuario cancela, salir del método
                    }
                    if (nuevoEmail.isBlank() || !nuevoEmail.matches("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,5}$")) {
                        JOptionPane.showMessageDialog(null, "El campo 'Email' debe tener el formato nombre@dominio.extension, y la extensión no debe superar los 5 caracteres.");
                    }
                } while (nuevoEmail != null && (nuevoEmail.isBlank() || !nuevoEmail.matches("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,5}$")));

                // Actualizar la base de datos
                try (Connection connection = DriverManager.getConnection(jdbcURL)) {
                    String updateQuery = "UPDATE Clientes SET cedula = ?, nombre_cliente = ?, apellido_cliente = ?, telefono = ?, email = ? WHERE cedula = ?";
                    PreparedStatement preparedStatement = connection.prepareStatement(updateQuery);
                    preparedStatement.setString(1, nuevoCedula);
                    ...
                    preparedStatement.setString(6, cedula); // Usamos la cédula antigua como criterio de búsqueda

                    int filasActualizadas = preparedStatement.executeUpdate();
                    if (filasActualizadas > 0) {
                        // Actualizar la tabla
                        modeloTabla.setValueAt(nuevoCedula, filaSeleccionada, 0);
                        modeloTabla.setValueAt(nuevoNombre, filaSeleccionada, 1);
                        modeloTabla.setValueAt(nuevoApellido, filaSeleccionada, 2);
                        modeloTabla.setValueAt(nuevoTelefono, filaSeleccionada, 3);
                        modeloTabla.setValueAt(nuevoEmail, filaSeleccionada, 4);
        ...
        ventanaUsuarios.add(botonActualizar, "South");
    ...
    private void mostrarInformeProductos() {
        ...
        botonActualizar.addActionListener(e -> {
            ...
            if (filaSeleccionada != -1) {
                ...
                String codigoProducto = modeloTabla.getValueAt(filaSeleccionada, 0).toString();

                // Validar cada campo con un bucle para permitir reintentos
                String nuevoNombre;
                do {
                    nuevoNombre = JOptionPane.showInputDialog("Nuevo Nombre del Producto:", modeloTabla.getValueAt(filaSeleccionada, 1).toString());
                    if (nuevoNombre == null || nuevoNombre.isBlank()) {
                        JOptionPane.showMessageDialog(null, "El campo 'Nombre Producto' no puede estar en blanco.");
                    } else if (!nuevoNombre.matches("[A-Za-z0-9áéíóúÁÉÍÓÚñÑ\\-\\_\\.\\s]+")) {
                        JOptionPane.showMessageDialog(null, "El campo 'Nombre Producto' solo puede contener letras, números y los caracteres especiales: '-', '_', '.', y espacios.");
                    }
                } while (nuevoNombre == null || nuevoNombre.isBlank() || !nuevoNombre.matches("[A-Za-z0-9áéíóúÁÉÍÓÚñÑ\\-\\_\\.\\s]+"));

                String nuevoPrecio;
                do {
                    nuevoPrecio = JOptionPane.showInputDialog("Nuevo Precio:", modeloTabla.getValueAt(filaSeleccionada, 2).toString());
                    if (nuevoPrecio == null || nuevoPrecio.isBlank()) {
                        JOptionPane.showMessageDialog(null, "El campo 'Precio' no puede estar en blanco.");
                    } else {
                        try {
                            Double.parseDouble(nuevoPrecio);  // Intentamos convertir a número
                        } catch (NumberFormatException ex) {
                            JOptionPane.showMessageDialog(null, "El campo 'Precio' debe ser un número válido.");
                            nuevoPrecio = null;  // Reiniciar si no es válido
                        }
                    }
                } while (nuevoPrecio == null || nuevoPrecio.isBlank());

                String nuevoStock;
                do {
                    nuevoStock = JOptionPane.showInputDialog("Nuevo Stock:", modeloTabla.getValueAt(filaSeleccionada, 3).toString());
                    if (nuevoStock == null || nuevoStock.isBlank()) {
                        JOptionPane.showMessageDialog(null, "El campo 'Stock' no puede estar en blanco.");
                    } else {
                        try {
                            Integer.parseInt(nuevoStock);  // Intentamos convertir a número entero
                        } catch (NumberFormatException ex) {
                            JOptionPane.showMessageDialog(null, "El campo 'Stock' debe ser un número entero.");
                            nuevoStock = null;  // Reiniciar si no es válido
                        }
                    }
                } while (nuevoStock == null || nuevoStock.isBlank());

                // Actualizar en la base de datos
                try (Connection connection = DriverManager.getConnection(jdbcURL)) {
                    String updateQuery = "UPDATE Productos SET nombre_producto = ?, precio = ?, cantidad_en_stock = ? WHERE id_producto = ?";
                    PreparedStatement preparedStatement = connection.prepareStatement(updateQuery);
                    preparedStatement.setString(1, nuevoNombre);
                    preparedStatement.setDouble(2, Double.parseDouble(nuevoPrecio));
                    preparedStatement.setInt(3, Integer.parseInt(nuevoStock));
                    preparedStatement.setString(4, codigoProducto);

                    int filasActualizadas = preparedStatement.executeUpdate();
                    if (filasActualizadas > 0) {
                        // Actualizar en el modelo de la tabla
                        modeloTabla.setValueAt(nuevoNombre, filaSeleccionada, 1);
                        modeloTabla.setValueAt(Double.parseDouble(nuevoPrecio), filaSeleccionada, 2);
                        modeloTabla.setValueAt(Integer.parseInt(nuevoStock), filaSeleccionada, 3);
        ...
        ventanaProductos.add(botonActualizar, "South");
